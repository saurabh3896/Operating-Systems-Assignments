...
int _do_set_deadline(message *m_ptr)
{
	...
	/* for non-preemptive version, make the time slices infinite so that no preemption occurs, runs
	like FCFS if same priorities */
	rmp->time_slice = INT_MAX;

	int add_factor = (m_ptr->m1_i1)/SCALE;
